...
{
    class RoomItemWiredHasFurniOn : RoomItemWiredCondition
    {
        public List<RoomItem> SelectedItems;
        public bool AllFurnis;

        public RoomItemWiredHasFurniOn(uint id, uint roomId, uint userId, Item baseItem, string extraData, int x, int y, double z, int rot, WallCoordinate wallCoordinate, Room room)
        ...
        public override bool IsBlocking(RoomUnitUser triggerer)
        {
            ...
            {
                if (item_.Z < this.Room.RoomGamemapManager.GetTile(item_.X, item_.Y).GetZ(false))
                {
                    if (!this.AllFurnis)
                    {
                        return false;
            ...
            if (this.AllFurnis && furniMatches == this.SelectedItems.Count)
            {
                return false;
            }
