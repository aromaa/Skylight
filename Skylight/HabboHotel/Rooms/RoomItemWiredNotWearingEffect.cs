...
{
    class RoomItemWiredNotWearingEffect : RoomItemWiredCondition
    {
        public string EffectID;

        public RoomItemWiredNotWearingEffect(uint id, uint roomId, uint userId, Item baseItem, string extraData, int x, int y, double z, int rot, WallCoordinate wallCoordinate, Room room)
        ...
        public override void OnUse(GameClient session, RoomItem item, int request, bool userHasRights)
        {
            if (userHasRights)
            {
                ...
                message.AppendString(this.EffectID); //data
