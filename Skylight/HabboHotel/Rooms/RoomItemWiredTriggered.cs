...
{
    class RoomItemWiredTriggered : RoomItemWiredCondition
    {
        public List<RoomItem> SelectedItems;
        public bool AllFurnis;

        public RoomItemWiredTriggered(uint id, uint roomId, uint userId, Item baseItem, string extraData, int x, int y, double z, int rot, WallCoordinate wallCoordinate, Room room)
        ...
        public override bool IsBlocking(RoomUnitUser triggerer)
        {
            if (triggerer != null)
            {
                int triggeredWireds = 0;
                foreach (RoomItem item_ in this.SelectedItems)
                {
                    if ((item_ as RoomItemWiredActionTrigger)?.TriggeredUsers.Contains(triggerer.UserID) == true)
                    {
                        if (!this.AllFurnis)
                        {
                            return false;
                ...
                if (this.AllFurnis && triggeredWireds == this.SelectedItems.Count)
                {
                    return false;
                }
            }
